...
class MIRAGE(tf.keras.Model):
    ...
    def get_glimpses(self, x_ind, y_ind):
        ...
        # Create meshgrid for gathering
        # self.x_mesh are the relative x coordinates for each pixel in the glimpse
        # x_ind are the absolute x coordinates for the center pixel of each glimpse

        # self.x_mesh[None, :, :] adds a new dimension at the start of the tensor
        # tf.tile replicates this tensor along the new dimension for each of the x_ind values
        # So if x_ind has shape (N,), then tf.tile will create a tensor of shape (N, H, W) where H and W are the height and width of the glimpse

        # x_ind has shape (N,), where N is the batch_size
        X = (
        ...
        # Combine X and Y into a single tensor along the last axis
        # XY should have the shape (batch_size, H, W, 2) where the last dimension contains the (x, y) coordinates
        ...
        # We do this for each image in the list of images
        # Then we concatenate the results along the first axis (num_images)
        # so pixel_glimpses_float will have shape (num_images * batch_size, H, W)
        # this is a typical gather, concat, split, stack, transpose pattern to handle multiple images
        ...
        # this is then reshaped to (num_images, batch_size, H, W)
        # then transposed to (batch_size, H, W, num_images)
        # doing the concat and split allows us to add the num_images as the first dimension
        # the num_images dimension is last for easier processing later on
        pixel_glimpses_float = tf.transpose(
            tf.stack(tf.split(pixel_glimpses_float, self.num_images, axis=0), axis=-1),
            [3, 1, 2, 0],
        )

        # the shape of pixel_glimpses_float is (batch_size, H, W, num_images)

        # Now do the same for the moving images (reference images)
        # gather_nd extracts values from the images at the specified coordinates
        # We do this for each image in the list of images
        # Then we concatenate the results along the first axis (num_images)
        # so pixel_glimpses_ref will have shape (num_images * batch_size, H, W)
        # this differs from pixel_glimpses_float which uses self.references because it uses offset without padding
        # we want the full glimpse for the float version but only the core region for the reference version
    ...
    def train(
        ...
        for _ in tq:
            ...
            #     save_path = f"/data1/peerd/spatial_datalake/roses3_pancreas_pdac_basal_ablation/xenium/deformation_images/heatmaps_ssim_nan/heatmap_step_{_:05d}.png"
